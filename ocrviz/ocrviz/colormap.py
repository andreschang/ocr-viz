...
class colormap(object):
    ...
    def make_grad(self, colors, nsteps=400, spacing = 'even', output = 'hex', rv_fix = 'on',\
    ...
    def make_cmap(self, colors, nsteps = 400, name = 'custom_map', rv_fix = 'on', edit = '', sequence = 'off', ngl = 'off'):

        if ngl == "on":

            hex_array = self.make_grad(colors, nsteps = nsteps, output = 'hex', rv_fix = rv_fix, name = name, sequence = 'off')

            if sequence == 'on':
                hsl_array, step1, hsls = self.make_grad(colors, nsteps = nsteps, output = 'hsl', rv_fix = rv_fix, name = name, sequence = 'on')
            ...
            if sequence == "off":
                hsl_array = self.make_grad(colors, nsteps = nsteps, output = 'hsl', rv_fix = rv_fix, name = name, sequence = 'off')
            else:
                hsl_array, step1, hsls = self.make_grad(colors, nsteps = nsteps, output = 'hsl', rv_fix = rv_fix, name = name, sequence = 'on')
    ...
    def cmap_sequence(self, all_mains, spacing = 'even', nmaps = 365, cycle = 'yes', rv_fix1 = "on", rv_fix2 = "off", ngl = 'off'):
        ...
        for sublist in range(ngroups):
            if len(all_mains[sublist]) < num_mains:
                ninsert = num_mains - 2
                fullgrad = self.make_grad(all_mains[sublist], output = 'hsl', rv_fix = 'off')
        ...
        for row in range(num_mains):
            ...
            horizontals.append(self.make_grad(row_mains, nsteps = nmaps, spacing = spacing, output = 'hex', rv_fix = rv_fix1))
        ...
        for day in range(nmaps):
            if rv_fix2 == 'on':
                cmap, step1, hsls = self.make_cmap(horizontals[day], name = str(day)+'_map', rv_fix = 'on', sequence = 'on', ngl = ngl)
            else:
                cmap, step1, hsls = self.make_cmap(horizontals[day], name = str(day)+'_map', rv_fix = 'off', sequence = 'on', ngl = ngl)
            ...
            if day == apply_when:
                sub_sequence = self.cmap_transition(allhsls[day-transitionl:day], rspan, transitionl, ngl = ngl)
    ...
    def cmap_transition(self, hsls, rspan, transitionl, ngl = 'off'):
        ...
        trans0 = self.cmap_sequence(all_mains0, nmaps = transitionl/2, cycle = 'no', rv_fix2 = 'off', ngl = ngl)
        trans1 = self.cmap_sequence(all_mains1, nmaps = transitionl/2, cycle = 'no', ngl = ngl)
