...
class colormap(object):
    ...
    def make_grad(self, colors, nsteps=400, spacing='even',
                  output='hex', rv_fix='on', name='custom_grad',
                  sequence='off', ngl='off'):
        ...
        for i in range(nmains):
            ...
            hsl_mains[i] = [mains[i].hue, mains[i].saturation,
                            mains[i].luminance]
        ...
        if spacing == 'even':
            for i in range(nspans):
                # if i == 1:
                    ...
                    # print hsl_step[i][0]
                for j in range(3):
                    # for hue transitions that cross between red and violet
                    if (rv_fix == 'on' and
                            j == 0 and np.abs(hsl_step[i][j]) > 0.5):
                        if hsl_step[i][j] > 0.5:
                            new_step = -(1.-hsl_step[i][j])
                            hsl_array[i*n:(i+1)*n, j] = np.concatenate(
                                (np.arange(hsl_mains[i][j], 0.0, new_step/n),
                                 np.arange(1.0, hsl_mains[i+1][j], new_step/n))
                                )[:n]
                        elif hsl_step[i][j] < -0.5:
                            new_step = -(-1.-hsl_step[i][j])
                            hsl_array[i*n:(i+1)*n, j] = np.concatenate(
                                (np.arange(hsl_mains[i][j],1.0, new_step/n),
                                 np.arange(0.0, hsl_mains[i+1][j], new_step/n))
                                )[:n]
                    else:
                        if hsl_step[i][j] == 0:
                            hsl_array[i*n:(i+1)*n, j] = hsl_mains[i][j] * \
                                np.ones(n)
                        else:
                            hsl_array[i*n:(i+1)*n, j] = np.arange(
                                hsl_mains[i][j], hsl_mains[i+1][j],
                                hsl_step[i][j]/n)[:n]
            ...
            for i in range(nspans):
                for j in range(3):
                    ...
                    if (rv_fix == 'on' and j == 0 and
                            np.abs(hsl_step[i][j]) > 0.5):
                        if hsl_step[i][j] > 0.5:
                            new_step = -(1.-hsl_step[i][j])
                            hsl_array[ni:ni+n[i], j] = np.concatenate(
                                (np.arange(hsl_mains[i][j], 0.0,
                                 new_step/n[i]), np.arange(1.0,
                                 hsl_mains[i+1][j], new_step/n[i])))[:n[i]]
                        elif hsl_step[i][j] < -0.5:
                            new_step = -(-1.-hsl_step[i][j])
                            hsl_array[ni:ni+n[i], j] = np.concatenate(
                                (np.arange(hsl_mains[i][j], 1.0,
                                 new_step/n[i]), np.arange(0.0,
                                 hsl_mains[i+1][j], new_step/n[i])))[:n[i]]
                    else:
                        hsl_array[ni:ni+n[i], j] = np.arange(
                            hsl_mains[i][j], hsl_mains[i+1][j],
                            hsl_step[i][j]/n[i])[:n[i]]

                ni += n[i]

        if output == 'hex':
            hsl_array = np.array([Color(
                hsl=(hsl_array[nn, 0], hsl_array[nn, 1], hsl_array[nn, 2])).hex
                     for nn in range(nsteps)])

        elif output == 'Color':
            hsl_array = np.array([Color(
                hsl=(hsl_array[nn, 0], hsl_array[nn, 1], hsl_array[nn, 2]))
                     for nn in range(nsteps)])
    ...
    def make_cmap(self, colors, nsteps=400, name='custom_map', rv_fix='on',
                  edit='', sequence='off', ngl='off'):

        if ngl == "on":
            hex_array = self.make_grad(
                colors, nsteps=nsteps, output='hex', rv_fix=rv_fix, name=name,
                sequence='off')

            if sequence == 'on':
                hsl_array, step1, hsls = self.make_grad(
                    colors, nsteps=nsteps, output='hsl', rv_fix=rv_fix,
                    name=name, sequence='on')
            ...
            if sequence == "off":
                hsl_array = self.make_grad(
                    colors, nsteps=nsteps, output='hsl', rv_fix=rv_fix,
                    name=name, sequence='off')
            else:
                hsl_array, step1, hsls = self.make_grad(
                    colors, nsteps=nsteps, output='hsl', rv_fix=rv_fix,
                    name=name, sequence='on')
            ...
            for i in range(nsteps):
                cpick = Color(hsl=(
                    hsl_array[i, 0], hsl_array[i, 1], hsl_array[i, 2]))
                cdict['red'].append(
                    (float(i)/nsteps, cpick.red, cpick.red))
                cdict['green'].append(
                    (float(i)/nsteps, cpick.green, cpick.green))
                cdict['blue'].append(
                    (float(i)/nsteps, cpick.blue, cpick.blue))

            cdict['red'].append(
                (1., cdict['red'][-1][1], cdict['red'][-1][2]))
            cdict['green'].append(
                (1., cdict['green'][-1][1], cdict['green'][-1][2]))
            cdict['blue'].append(
                (1., cdict['blue'][-1][1], cdict['blue'][-1][2]))

            cdict['red'], cdict['green'], cdict['blue'] = tuple(
                cdict['red']), tuple(cdict['green']), tuple(cdict['blue'])
    ...
    def cmap_sequence(self, all_mains, spacing='even', nmaps=365, cycle='yes',
                      rv_fix1="on", rv_fix2="off", ngl='off'):

        # all_mains is a list of each combination of colors (high, mid, low)
        # ex. [[winter colors], [spring colors],
        # [summer colors], [spring colors]]
        ...
        for sublist in range(ngroups):
            if len(all_mains[sublist]) < num_mains:
                ninsert = num_mains - 2
                fullgrad = self.make_grad(all_mains[sublist],
                                          output='hsl', rv_fix='off')
                midpoints = [(n+1)*int(fullgrad.shape[0]/(ninsert+1)) for
                             n in range(ninsert)]
                print('ADDING MAINS at ... '+str(midpoints))
                mcolors = [Color(hsl=(
                    fullgrad[midpoints[nn], 0], fullgrad[midpoints[nn], 1],
                    fullgrad[midpoints[nn], 2])) for nn in range(ninsert)]
                all_mains[sublist] = [all_mains[sublist][0],
                                      all_mains[sublist][-1]]
        ...
        for row in range(num_mains):
            ...
            horizontals.append(self.make_grad(row_mains, nsteps=nmaps, 
                               spacing=spacing, output='hex', rv_fix=rv_fix1))
        ...
        for day in range(nmaps):
            if rv_fix2 == 'on':
                cmap, step1, hsls = self.make_cmap(
                    horizontals[day], name=str(day)+'_map', rv_fix='on',
                    sequence='on', ngl=ngl)
            else:
                cmap, step1, hsls = self.make_cmap(
                    horizontals[day], name=str(day)+'_map', rv_fix='off',
                    sequence='on', ngl=ngl)
            ...
            for i in range(len(step1)):
                ...
                if step0[i]*step1[i] < -0.05:
                    print("APPLY TRANSITION across break at "+str(day-1) +
                          " - "+str(day))
            ...
            if day == apply_when:
                sub_sequence = self.cmap_transition(
                    allhsls[day-transitionl:day], rspan, transitionl, ngl=ngl)
    ...
    def cmap_transition(self, hsls, rspan, transitionl, ngl='off'):
        ...
        mainsb = [Color(hsl=(hsls[0][m][0], hsls[0][m][1], hsls[0][m][2])).hex
                  for m in range(len(hsls[0]))]
        ...
        mainsf = [Color(
            hsl=(hsls[-1][m][0], hsls[-1][m][1], hsls[-1][m][2])).hex
            for m in range(len(hsls[0]))]
        all_mains0 = np.concatenate(([mainsb], [mains0]))
        all_mains1 = np.concatenate(([mains1], [mainsf]))

        trans0 = self.cmap_sequence(all_mains0, nmaps=transitionl/2,
                                    cycle='no', rv_fix2='off', ngl=ngl)
        trans1 = self.cmap_sequence(all_mains1, nmaps=transitionl/2,
                                    cycle='no', ngl=ngl)
        trans_maps = trans0+trans1

        return trans_maps

    def test_cmap(self, cmap, fname='test_cmap'):
